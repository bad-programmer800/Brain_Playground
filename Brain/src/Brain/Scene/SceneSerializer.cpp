...
namespace Brain
{
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		BR_CORE_TRACE("Deserializing Scene '{0}'", sceneName);
		...
		if (entities)
		{
			...
			{
				uint64_t uuid = entity["Entity"].as<uint64_t>(); // To Do
				...
				BR_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
