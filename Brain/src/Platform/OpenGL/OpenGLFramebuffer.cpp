...
namespace Brain {
	...
	namespace Utils
	{
		...
		static void AttachDepthTexture(uint32_t id, int samples,
		...
		static GLenum BrainFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	OpenGLFramebuffer::~OpenGLFramebuffer()
	{
		...
		glDeleteTextures(m_ColorAttachments.size(),
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_DepthAttachmentSpecification.TextureFormat != FramebufferTextureFormat::None)
		{
			...
			case FramebufferTextureFormat::DEPTH24STENCIL8:
				Utils::AttachDepthTexture(m_DepthAttachment,
					m_Specification.Samples, GL_DEPTH24_STENCIL8,
		...
		{
			BR_CORE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		BR_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			BR_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		BR_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::BrainFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}